...
\section*{Activities}
1. We perform the first task as follows:
```{r}
Carseats_split <- initial_split(Carseats, strata = Sales, prop = 0.8)

Carseats_train <- training(Carseats_split)
Carseats_test <- testing(Carseats_split)
```
2. We perform the second task as follows:
```{r}
dim(Carseats_train)
dim(Carseats_test)
```
We have done both tasks and verified that they are correct.
...
\section*{Activities}
We do the first activity as follows:
```{r}
lm_spec2 <- linear_reg() %>%
  set_mode("regression") %>%
  set_engine("lm")
...
```
We do the second activity as follows:
```{r}
augment(lm_fit2, new_data = Carseats_train) %>%
  rmse(truth = Sales, estimate = .pred)

augment(lm_fit2, new_data = Carseats_test) %>%
  rmse(truth = Sales, estimate = .pred)
```
Both values seem to be around 1.00. 
...
\section*{Activities}
1. We do the first activity as follows:
```{r}
poly_rec2 <- recipe(Sales ~ ., data = Carseats_train) %>%
  step_poly(., degree = 2)

poly_wf2 <- workflow() %>%
  add_recipe(poly_rec2) %>%
  add_model(lm_spec2)

poly_fit2 <- fit(poly_wf2, data = Carseats_train)
```
2. We do the second activity as follows:
```{r}
augment(poly_fit2, new_data = Carseats_train) %>%
  rmse(truth = Sales, estimate = .pred)

augment(poly_fit2, new_data = Carseats_test) %>%
  rmse(truth = Sales, estimate = .pred)
```
...
\section*{Activities}
1. We repeat all of these steps on the 'Carseats' data as follows:
```{r}
poly_tuned_rec2 <- recipe(Sales ~ ., data = Carseats_train) %>%
  step_poly(., degree = tune())

poly_tuned_wf2 <- workflow() %>%
  add_recipe(poly_tuned_rec2) %>%
  add_model(lm_spec2)

Carseats_folds <- vfold_cv(Carseats_train, v = 10)

degree_grid2 <- grid_regular(degree(range = c(1, 10)), levels = 10)

tune_res2 <- tune_grid(
  object = poly_tuned_wf2, 
  resamples = Carseats_folds, 
  grid = degree_grid2
)

best_degree2 <- select_by_one_std_err(tune_res2, degree, metric = "rmse")

final_wf2 <- finalize_workflow(poly_wf2, best_degree2)

final_fit2 <- fit(final_wf2, Carseats_train)

augment(final_fit2, new_data = Carseats_test) %>%
  rmse(truth = Sales, estimate = .pred)
```
The degree that results in the best fitting model for the Carseats data is Degree 1.
...
```{r}
Carseats_boots <- bootstraps(Carseats)

boot.fn2 <- function(split) {
  lm_fit2 <- lm_spec2 %>% fit(Sales ~ ., data = analysis(split))
  tidy(lm_fit2)
}
  
boot_res <- Carseats_boots %>%
  mutate(models = map(splits, boot.fn2))
