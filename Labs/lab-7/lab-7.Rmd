...
The most important predictor of shelving location is if it's bad or medium. I can tell by looking at the nodes that have shelving location as the splitting variable, and noting that it only checks to see if the shelving location is bad or medium.
...
\subsubsection*{Answer}
The model performs quite a bit worse on the testing set than on the training set. This is because the tree might be overfit to the data on the training set, as classification trees normally are overfit to the data they are trained on. In particular, it performs about 16 percent worse. 
...
\subsubsection*{Answer}
Looking at the plot, I notice that the accuracy stays relatively stable when ranging from 0.001 to 0.04, then jumps up for a bit from 0.04 to 0.06, then drops very low until about 0.08, then goes back up a bit when at 0.1. This indicates to me that the model performs best when the cost complexity value is inbetween 0.04 and 0.06. 
...
\subsubsection*{Answer}
The default amount of folds created is v = 10.
...
\subsubsection*{Answer}
I notice that as the cost complexity parameter increases, the root mean square error also increases. Thus, as performance decreases, the cost complexity (penalty) incresaes. 
...
\subsubsection*{Answer}
I noticed that the fit took longer to run, but it did end up resulting in a better performing model, by quite a bit.
...
\subsubsection*{Answer}
Based on the code above, we can see that a random forest performed the best, as it had the lowest RMSE.
