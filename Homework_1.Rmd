...
library(forcats)
```
Then, we can create the bar plot as requested:
```{r}
ggplot(mpg, aes(x = manufacturer)) + geom_bar(stat = "count") + coord_flip()
```
...
```
In particular, the error is because 'manufacturer' is type 'character'. As suggested by Hanmo Li in office hours, to resolve this issue, we need to remove 'manufacturer' from the dataset.
```{r}
new_mpg <- mpg
