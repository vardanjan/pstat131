...
#Then, we want to code the dummy variables for any categorical predictors. We note that 'type', which indicates the sex, is the only categorical predictor.
age_recipe <- 
  age_recipe %>%
  step_dummy('type') %>%
  prep(training = new_dataset_train)
...
input = data.frame(c('F', 0.5, 0.10, 0.30, 4, 1, 2, 1))
